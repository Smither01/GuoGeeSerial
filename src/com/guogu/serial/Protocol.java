		...
		public void run() {
			...
			//		dataPackage.add(GetGateWayFrame.GetStrData());
				...
				while (m_ThreadAliveFlag && null != m_GatewayMac) {
					...
					if (!m_RemoteFlag) {
						Log.v("Protocol", "! m_RemoteFlag server success");
						...
						Log.v("Protocol", "Remote server success");
						boolean flag = true;
			...
			Log.v("Protocol", "thread KeepAlive exit");
		}
	}
	//返回遍历查询节点状�?�指�?
...
//				+ NetFrame.GetVer() + " fun:" + (NetFrame.GetFun() & 0xff));
		if (FilterKeepAliveRet(NetFrame)) {
			Log.v("Protocol", "FilterKeepAliveRet");
			return;
		} else if (FilterCtlServerGet(NetFrame)) {
			Log.v("Protocol", "FilterCtlServerGet");
			return;
		}
		...
		{
			if(NetFrame.GetFun() == 0x08){//若是返回节点状�?�包
				try{
					...
					for (int m = 0; m < length/80 +1; m++) {		
						...
						//    	byte[] queryByte = new byte[46 + length*5];
								System.arraycopy(getSourceByte, 0, queryByte, 0, 2);//�?	
						//		byte[] queryByteLength = Util.Short2Byte((short)(0x2E + length*5));
								...
								queryByte[34] = (byte)(length/80 +1);//帧长�?
								queryByte[35] = (byte)(m+1);//当前帧序�?
	...
	/*	byte[] tmp = NetFrame.GetStrData();
		...
		if (!SerialComm.getInstance().getUsbNotExist()) {
			...
			Log.v("Protocol", "dataPackage Size :"+dataPackage.size());
		} else {
			if (dataPackage.size() > 0)
				dataPackage.clear();
